...
IGNORE_DIRS = {".obsidian", ".trash", ".git", "_meta"}  # _meta = tooling/reports, not content
...
def digest_report(vault, window_days):
    ...
    def has_digest(txt: str) -> bool:
        ...
        for inline in re.findall(r"^tags:\s*\[([^\]]*)\]", block, re.M):
            if any(t.strip().strip("\"'").lstrip("#") == "digest"
                   for t in inline.split(",")):
                return True
        ml = re.search(r"^tags:\s*\n((?:\s*-\s*.+\n?)+)", block, re.M)
        if ml and any(l.strip().lstrip("-").strip().strip("\"'").lstrip("#") == "digest"
    ...
    def is_finding(f: Path) -> bool:
        ...
        return (("permanent/" in rel or "literature/" in rel)
                and not f.name.startswith("00-") and f.name.lower() != "readme.md")

    findings = [f for f in iter_files(vault)
                if f.suffix.lower() == ".md" and is_finding(f)]
    ...
    cur_ts = None
    cur_files: list[str] = []
    for line in git("log", "--diff-filter=A", "--pretty=format:@%ct",
    ...
    print(f"finding notes: {len(findings)}   window: last {window_days} days   "
...
UNCERTAINTY_RE = re.compile(r"(CI|confidence interval|\binterval\b|uncertainty|standard error|\bstd\b|±)", re.I)
REPLICATED_RE = re.compile(r"(replicat|across (seeds|models|datasets)|independent approach|\bseeds\b)", re.I)
PAPER_GRADE_RE = re.compile(r"(independent verification|independently verified|independent check|\bexternal\b)", re.I)
...
def _is_graded_permanent(f: Path, vault: Path, fm_block: str) -> bool:
    rel = str(f.relative_to(vault)).lower()
    if "permanent/" not in rel or f.name.startswith("00-") or f.name.lower() == "readme.md":
        return False
...
def _preregistration_missing(body: str) -> list[str]:
    ...
    missing = []
    for label, rx in (("Test", HYP_TEST_RE), ("Threshold", HYP_THRESHOLD_RE),
...
def _grade_markers_missing(grade: str, body: str) -> list[str]:
    ...
    if g == "robust":
        ...
        if not UNCERTAINTY_RE.search(body):
            missing.append("an uncertainty token (CI/interval/uncertainty/standard error/std/±)")
        return missing
    if g == "replicated":
        return [] if REPLICATED_RE.search(body) else \
            ["a replication token (replicat.../across seeds|models|datasets/independent approach/seeds)"]
    if g == "paper-grade":
        return [] if PAPER_GRADE_RE.search(body) else ["an independent-verification token"]
    return []
...
def _hypothesis_findings(vault: Path, stale_days: int, today: date) -> dict:
    ...
    for f in md:
        ...
        is_graded_permanent = (not is_dossier) and _is_graded_permanent(f, vault, fm_block)
    ...
    return {"open": open_list, "stale": stale_list, "integrity": integrity_list,
...
def main() -> int:
    ...
    ap.add_argument("--hyp-stale-days", type=int, default=21,
    ...
    p(f"- words: total {sum(wc):,}   median {wc[n // 2]:,}   "
      f"p90 {wc[int(n * 0.9)]:,}   max {wc[-1]:,}")
    p(f"- links: {path_links + filename_links} "
      f"({path_links} path-based, {filename_links} filename-only)")
    p(f"- **broken links: {len(broken)}**")
    p(f"- orphans: {len(orphans)} total, **{len(idea_orphans)} idea-bearing** "
      f"(non-log/reference)")
    ...
    dump("Idea-bearing orphans (candidates to link or promote)", idea_orphans,
         lambda f: str(f.relative_to(vault)))
    dump("Oversized idea notes (candidates to split, only if idea, not log)",
         sorted(oversized, key=lambda f: -words[f]),
         lambda f: f"{words[f]:>6,}w  {f.relative_to(vault)}")
    dump("Version-duplicate clusters (archive superseded)",
